...
public abstract class BaseAgent {
    ...
    public String run(String userPrompt) {
        ...
        try {
            for (int i = 0; i < maxSteps && state != AgentState.FINISHED && state != AgentState.STUCK; i++) {
                ...
                // 每轮 step 前检测是否陷入循环；若检测到则注入提示词（极多次仍无效则强制终止）
                checkAndHandleStuck();
                if (state == AgentState.STUCK) {
                    results.add("执行结束：检测到循环，已强制终止（连续" + stuckCount + "次重复响应）");
                    break;
                }
    ...
    protected void checkAndHandleStuck() {
        if (!isStuck()) {
            ...
            // 注意：这里只在循环开头清零，stuck 路径中由 handleStuckState 自己 ++。
            stuckCount = 0;
            return;
        }
        handleStuckState();
     ...
     * <p>实现细节：
     * <ul>
     *   <li>少于 2 条消息直接认为没卡</li>
     *   <li>最后一条不能为空（避免空文本被误判为卡死）</li>
     *   <li>用 equals 严格比较内容（忽略大小写 / 标点的近似匹配不是当前目的）</li>
     * </ul>
     */
